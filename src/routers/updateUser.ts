...
router.put(
    ...
    [
        ...
        body('first_name').isString().withMessage('Email must be valid'),
        body('last_name').isString().withMessage('Email must be valid'),
        body('contact').isString().withMessage('Email must be valid'),
        body('birth_date').isDate().withMessage('Email must be valid'),
    ],
    validateRequest,
    async (req: Request, res: Response) => {
        try {
            ...
            if (user) {
                ...
                await user.save();
                return successHandler(res, 202, "User updated...");
            } else {
